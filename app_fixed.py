...
def interpret(text):
    ...
    def num(pattern, default=0):
        ...
        for w,n in {'one':1,'two':2,'three':3,'four':4,'five':5,'six':6,'seven':7,'eight':8}.items():
    ...
    # A conventional non-hub VNet is required when requirements say one VNet with subnets.
    default_vnet=root
    if not re.search(r'\bhub\b',t) and ('vnet' in t or 'subnet' in t): container('vnet','Virtual Network',root,'vnet'); default_vnet='vnet'
    hub=bool(re.search(r'\bhub\b',t))
    if hub:
        container('hub','Hub VNet',root,'vnet')
    ...
    for s in range(1,spoke_count+1):
        sid=f'spoke{s}'; container(sid,f'Spoke VNet {s}',root,'vnet'); sub=f'appsub{s}'; container(sub,'Application Subnet',sid,'subnet')
    ...
    for key,label in [('frontend','Frontend Subnet'),('backend','Backend Subnet'),('data','Data Subnet'),('management','Management Subnet')]:
        if key+' subnet' in t: container(key+'subnet',label,'hub' if hub and key=='management' else default_vnet,'subnet')
    frontend='frontendsubnet' if any(c['id']=='frontendsubnet' for c in containers) else default_vnet
    backend='backendsubnet' if any(c['id']=='backendsubnet' for c in containers) else default_vnet
    data='datasubnet' if any(c['id']=='datasubnet' for c in containers) else default_vnet
    ...
    ids={n['id'] for n in nodes}; chain=[x for x in ['frontdoor','appgw','lb','apim','webapp','aks'] if x in ids]
    for a,b in zip(chain,chain[1:]): edge(a,b,'HTTPS')
    if 'webapp' in ids and 'function' in ids: edge('webapp','function','Invoke')
    ...
    if not nodes: node('workload','Workload','app',default_vnet)
...
def pycode(model):
    ...
    lines=[imports,f'with Diagram("Generated Architecture", filename="diagrams/generated_architecture", outformat="png", show=False, direction="{direction}", graph_attr={{"compound":"true","splines":"spline","nodesep":"0.9","ranksep":"1.1","pad":"0.4"}}):','    n = {}','    anchors = {}']
    def emit(parent,indent):
        ...
        for c in children.get(parent,[]):
            # Each VNet/subnet is a distinct Cluster. Its Azure icon is an anchor in the top-right area of that box.
            lines.append(' '*indent+f'with Cluster("{c["label"]}", graph_attr={{"margin":"24","style":"rounded","labeljust":"l"}}):')
            icon='Subnets' if c.get('kind')=='subnet' else 'VirtualNetworks'
            label='Subnet' if c.get('kind')=='subnet' else ('VNet' if c.get('kind')=='vnet' else '')
            lines.append(' '*(indent+4)+f'anchors["{c["id"]}"] = {icon}("{label}")')
            emit(c['id'],indent+4)
    emit(None,4)
...
# Draw.io containers explicitly show VNet/Subnet badges at top-right and keep sibling subnets separate.
def drawio_bordered(model):
    import html
    esc=lambda s: html.escape(str(s),quote=True); cells=['<mxCell id="0"/>','<mxCell id="1" parent="0"/>']; children={}; nodechildren={}
    for c in model['containers']: children.setdefault(c.get('parent'),[]).append(c)
    for n in model['nodes']: nodechildren.setdefault(n.get('parent'),[]).append(n)
    def size(cid):
        cc=children.get(cid,[]); nn=nodechildren.get(cid,[])
        if not cc:return (360,max(190,110+85*len(nn)))
        ss=[size(x['id']) for x in cc]; return (max(440,max(w for w,h in ss)+60),120+sum(h for w,h in ss)+30*max(0,len(ss)-1)+85*len(nn))
    def emit(parent,parentcell):
        y=45
        for c in children.get(parent,[]):
            w,h=size(c['id']); kind=c.get('kind','vnet'); stroke='#0078D4' if kind=='vnet' else '#5B5FC7'; width='3' if kind=='vnet' else '2'
            cells.append(f'<mxCell id="{c["id"]}" value="{esc(c["label"])}" style="swimlane;html=1;rounded=1;startSize=34;horizontal=1;container=1;collapsible=0;strokeColor={stroke};strokeWidth={width};fillColor=none;fontStyle=1;" vertex="1" parent="{parentcell}"><mxGeometry x="30" y="{y}" width="{w}" height="{h}" as="geometry"/></mxCell>')
            badge='SUBNET' if kind=='subnet' else ('VNET' if kind=='vnet' else 'AZURE')
            cells.append(f'<mxCell id="badge_{c["id"]}" value="{badge}" style="rounded=1;whiteSpace=wrap;html=1;fontSize=10;fontStyle=1;strokeColor={stroke};fillColor=#ffffff;" vertex="1" parent="{c["id"]}"><mxGeometry x="{w-82}" y="6" width="68" height="24" as="geometry"/></mxCell>')
            emit(c['id'],c['id']); y+=h+30
        ny=55
        for n in nodechildren.get(parent,[]):
            cells.append(f'<mxCell id="{n["id"]}" value="{esc(n["label"])}" style="rounded=1;whiteSpace=wrap;html=1;fillColor=#ffffff;strokeColor=#0078D4;" vertex="1" parent="{parentcell}"><mxGeometry x="55" y="{ny}" width="210" height="60" as="geometry"/></mxCell>'); ny+=82
    emit(None,'1')
...
app.view_functions['generate'].__globals__['drawio_xml']=drawio_bordered
...
def regression(requirement):
    ...
    try:
        ...
        d=os.path.join(OUT,'regression.drawio'); open(d,'w').write(drawio_bordered(model))
        if not os.path.exists(png) or os.path.getsize(png)==0:return {'status':'failed','stage':'artifacts'},500
