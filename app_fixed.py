...
# Override the broken generator in app.py with imports verified against the
# repository's working contoso_architecture.py and diagrams==0.24.4.
def pycode(model):
    ...
    cls = {
        "firewall":"Firewall", "app":"AppServices", "function":"FunctionApps",
        "servicebus":"ServiceBus", "frontdoor":"FrontDoors", "appgw":"ApplicationGateway",
        "keyvault":"KeyVaults", "sql":"SQLDatabases", "storage":"StorageAccounts",
        "monitor":"ApplicationInsights", "bastion":"VirtualNetworks", "vpn":"VirtualNetworkGateways"
    }
    ...
    def emit(parent, indent):
        for n in nodechildren.get(parent, []):
            klass=cls.get(n['type'],'AppServices')
...
# Replace the function looked up by app.generate at runtime.
...
def regression_contoso():
    ok, detail=graphviz_check()
    if not ok: return {"status":"failed","stage":"graphviz","error":detail},500
    try:
        model=interpret(CONTOSO); code=pycode(model)
        p=os.path.join(BASE,'regression_contoso_generated.py')
        open(p,'w').write(code)
        ...
        if run.returncode != 0:
            return {"status":"failed","stage":"python","stderr":run.stderr[-3000:],"code":code},500
        open(os.path.join(OUT,'regression_contoso.drawio'),'w').write(drawio_xml(model))
        return {"status":"passed","graphviz":detail,"png_exists":os.path.exists(png),"png_bytes":os.path.getsize(png) if os.path.exists(png) else 0,"drawio_exists":os.path.exists(os.path.join(OUT,'regression_contoso.drawio')),"nodes":len(model['nodes']),"containers":len(model['containers']),"edges":len(model['edges'])}
    except Exception as e:
        return {"status":"failed","stage":"exception","error":repr(e)},500
